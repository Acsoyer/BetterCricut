...
type TextRequest = { mode: "text"; lines: string[]; font: "mixed" | "cursive" | "serif"; extraPrompt?: string; variation?: boolean; connectEverything?: boolean };
...
export async function POST(request: Request) {
  ...
  if (body.mode === "text") {
    ...
    prompt = `Create a single-color cake-topper lettering design as a transparent-background PNG. Render ONLY the requested lettering in near-black ink (#252826); no cake, stick, border, frame, scenery, mockup, paper, shadow, texture, background, or extra words. Preserve the spelling, capitalization, line order and line breaks EXACTLY. Center the composition and keep every letter fully inside the canvas with generous transparent padding. Balance the visual width of the lines: when one line contains much less text than another (for example \"Hello\" above \"30\", or \"Happy Birthday\" above \"Can\"), make the shorter line noticeably larger so the width difference is less extreme, but do NOT force the lines to exactly equal widths. Preserve a natural typographic hierarchy. The letterforms must be smooth, clean, connected where practical, and suitable for conversion into a Cricut cut path. ${fontDirections[body.font]} ${body.connectEverything ? "CONNECT EVERYTHING: the finished lettering must form one physically connected cuttable silhouette. Join letters or words already within 10 pixels using natural overlaps; bridge anything farther apart with tasteful structural flourishes. Every connection must be at least 10 pixels thick at 1024×1024 output (approximately 1 mm in the intended topper), never a hairline, while all text remains easy to read." : ""} ${body.variation ? "Create a distinctly different font treatment from the previous version while following every other instruction." : ""}\n\nEXACT TEXT:\n${exactText}${body.extraPrompt?.trim() ? `\n\nOptional decorative instruction, only if it does not alter the exact text: ${String(body.extraPrompt).trim().slice(0, 240)}` : ""}`;
