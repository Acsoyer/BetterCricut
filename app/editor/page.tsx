...
export default function Home() {
  const [layers, setLayers] = useState<Layer[]>([]),
    ...
    [imageTab,setImageTab]=useState<"edit"|"background">("edit"),
  ...
  const noBackground = (chosen:Layer|null=one) => {
    if (!chosen) return;
  ...
  const applyBackgroundPreset=async(type:"image"|"rim"|"text",chosen:Layer|null=one)=>{
    ...
    try{
      ...
      const t=await trimTransparent(refined),nextWidth=before.w*t.width,safety=await analyzeCutSafety(t.src,nextWidth),next={...chosen,...safety,src:t.src,x:before.x+before.w*t.left,y:before.y+before.h*t.top,w:nextWidth,h:before.h*t.height,kind:"nobg" as Kind},label=type==="image"?"Image Remove Background":type==="rim"?"Image Background + Rim":"Text Background Removal",
      ...
      mutate(chosen.id,()=>({...next,steps:[...steps,step],activeStep:steps.length}));setNotice(`${label} applied`)
  ...
  return (
    ...
    >
      ...
      {imageEditor&&(()=>{const target=layers.find(l=>l.id===imageEditor.layerId);return <div className="bg-modal image-edit-modal" role="dialog" aria-modal="true" aria-label="Image editor">
        <div className="bg-dialog"><header><div><b>Edit Image</b><small>Crop, erase or refine the image background.</small></div>{bgEditor&&<div className="refine-quick-tools image-refine-tools"><button disabled={imageTab==="edit"?!imageEditor.history.length:!bgEditor.strokes.length} onClick={()=>imageTab==="edit"?undoImageStage():setBgEditor({...bgEditor,strokes:bgEditor.strokes.slice(0,-1)})}>Undo</button><button disabled={imageTab==="edit"?!imageEditor.history.length:!bgEditor.strokes.length} onClick={()=>imageTab==="edit"?setImageEditor({...imageEditor,history:[],strokes:[]}):setBgEditor({...bgEditor,strokes:[]})}>Reset</button><label><input type="checkbox" checked={bgEditor.alphaView} onChange={e=>setBgEditor({...bgEditor,alphaView:e.target.checked})}/> Alpha</label><button className={bgEditor.optimizeAlpha?"active optimize-alpha":"optimize-alpha"} onClick={()=>setBgEditor({...bgEditor,optimizeAlpha:!bgEditor.optimizeAlpha,alphaView:true})}><Sparkles/> Optimize Alpha</button></div>}<button onClick={()=>{setImageEditor(null);setBgEditor(null)}}>×</button></header>
        <div className="cutout-tabs image-tabs"><button className={imageTab==="edit"?"active":""} onClick={()=>setImageTab("edit")}>1. Edit Image</button><button className={imageTab==="background"?"active":""} onClick={()=>setImageTab("background")}>2. Advanced Background Removal</button></div>
