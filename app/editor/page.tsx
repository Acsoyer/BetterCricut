...
type SplitPreview = {layerId:string;preview:string;parts:SplitPart[];base?:{x:number;y:number;w:number;h:number}};
type CutoutEditor = {
  layerId: string;
  source: string;
  color: string;
...
async function addProtectiveRim(src:string,physicalWidthCm:number){
  const img=await getImage(src),radius=clamp(Math.ceil(img.naturalWidth/Math.max(.1,physicalWidthCm)*.11),2,72),out=document.createElement("canvas");
  out.width=img.naturalWidth+radius*2;out.height=img.naturalHeight+radius*2;const x=out.getContext("2d")!;
  x.imageSmoothingEnabled=true;x.imageSmoothingQuality="high";
  // White is the safe fallback. Repeated source draws then replace it with real edge pixels wherever available.
  x.fillStyle="#fff";for(let angle=0;angle<Math.PI*2;angle+=Math.PI/24){const dx=Math.cos(angle)*radius,dy=Math.sin(angle)*radius;x.save();x.translate(radius+dx,radius+dy);x.drawImage(img,0,0);x.globalCompositeOperation="source-in";x.fillRect(0,0,img.naturalWidth,img.naturalHeight);x.restore()}
  x.globalCompositeOperation="source-over";for(let distance=radius;distance>=0;distance-=Math.max(1,radius/8))for(let angle=0;angle<Math.PI*2;angle+=Math.PI/24)x.drawImage(img,radius+Math.cos(angle)*distance,radius+Math.sin(angle)*distance);
  x.drawImage(img,radius,radius);return{src:out.toDataURL("image/png"),left:-radius/img.naturalWidth,top:-radius/img.naturalHeight,width:out.width/img.naturalWidth,height:out.height/img.naturalHeight};
}
...
export default function Home() {
  ...
  const saveProject = async (asNew = false,targetId?:string,targetName?:string,closePanel=true) => {
    ...
    let {data,error}=updateId&&!asNew
      ?await supabase.from("projects").update({name,data:projectData,updated_at:payload.updated_at}).eq("id",updateId).eq("user_id",session.user.id).select("id,name,updated_at,data").single()
      :await supabase.from("projects").insert(payload).select("id,name,updated_at,data").single();
    if((error||!data)&&updateId&&!asNew){const listed=projects.find(project=>project.id===updateId)||projects.find(project=>project.name===name);if(listed&&listed.id!==updateId)({data,error}=await supabase.from("projects").update({name,data:projectData,updated_at:payload.updated_at}).eq("id",listed.id).eq("user_id",session.user.id).select("id,name,updated_at,data").single())}
  ...
  const newProject=()=>{if(layers.length||currentProjectId){setPendingNewProject(true);return}createNewProject()};
  ...
  useEffect(()=>{const clearMarquee=()=>{setMarquee(null);setDrag(value=>value?.mode==="marquee"?null:value)};window.addEventListener("pointerup",clearMarquee);window.addEventListener("pointercancel",clearMarquee);window.addEventListener("blur",clearMarquee);return()=>{window.removeEventListener("pointerup",clearMarquee);window.removeEventListener("pointercancel",clearMarquee);window.removeEventListener("blur",clearMarquee)}},[]);
  ...
  const noBackground = (chosen:Layer|null|undefined=one) => {
    if (!chosen) return;
    const priorRemoval=[...chosen.steps].reverse().find(step=>step.type==="remove-bg"&&step.before),base=priorRemoval?.before||snapshot(chosen),settings=priorRemoval?.removalSettings;
    setBgPreview("");
  ...
  const buildBackgroundPreset=async(type:"image"|"rim"|"text",chosen:Layer)=>{
      const removalIndex=chosen.steps.findIndex(step=>step.type==="remove-bg"),prior=removalIndex>=0?chosen.steps[removalIndex]:undefined,before=prior?.before||snapshot(chosen),source=before.src;
      ...
      else{strokes=[{id:uid(),mode:"remove",brush:4,bleed:10,reach:null,points:[{x:.005,y:.005}]}];if(type==="rim")edgeSmooth=8}
      let refined=await refineBackground(source,0,strokes,0,edgeRefine,colors,edgeSmooth,true),map={left:0,top:0,width:1,height:1};
      if(type==="rim"){const rim=await addProtectiveRim(refined,before.w);refined=await optimizeAlphaChannel(rim.src);map={left:rim.left,top:rim.top,width:rim.width,height:rim.height}}
      if(type!=="text")refined=await featherAlphaInside(refined);
      const t=await trimTransparent(refined),left=map.left+map.width*t.left,top=map.top+map.height*t.top,width=map.width*t.width,height=map.height*t.height,nextWidth=before.w*width,safety=await analyzeCutSafety(t.src,nextWidth),next={...chosen,...safety,src:t.src,x:before.x+before.w*left,y:before.y+before.h*top,w:nextWidth,h:before.h*height,kind:"nobg" as Kind},label=type==="image"?"Image Remove Background":type==="rim"?"Image Background + Rim":"Text Background Removal",
        removalSettings={strokes,speckles:0,edgeRefine,edgeSmooth,optimizeAlpha:true,eraseColors:colors},step:LayerStep={id:uid(),type:"remove-bg",label,before,backgroundColor,removalSettings,snapshot:snapshot(next)},steps=removalIndex>=0?chosen.steps.slice(0,removalIndex):chosen.steps;
      return{...next,cutRisk:type==="rim"?false:next.cutRisk,cutRiskReason:type==="rim"?"":next.cutRiskReason,steps:[...steps,step],activeStep:steps.length};
  };
  const applyBackgroundPreset=async(type:"image"|"rim"|"text",chosen:Layer|null=one)=>{
    if(!chosen)return setNotice("Select one image first");setBgMenuOpen(false);setWorking(true);setNotice("Applying background removal preset…");
    try{
      const next=await buildBackgroundPreset(type,chosen);mutate(chosen.id,()=>next);setRiskLayerId(null);setNotice(`${type==="image"?"Image Remove Background":type==="rim"?"Image Background + Rim":"Text Background Removal"} applied`)
  ...
  const commitBackground = async () => {
    ...
    try {
      ...
      if(imageEditor?.layerId===target.id){setImageEditor({...imageEditor,source:t.src,strokes:[],crop:{left:0,top:0,right:0,bottom:0}});setImageTab("edit")}
      setBgEditor(null);
  ...
  const openImageEditor=(chosen:Layer|null=one)=>{if(!chosen||["vector","stroke","acetate"].includes(chosen.kind))return;setImageEditorSize({w:0,h:0});setImageTab("edit");setImageEditor({layerId:chosen.id,source:chosen.src,crop:{left:0,top:0,right:0,bottom:0},upscale:1,tool:"crop",brush:4,strokes:[],history:[],offsetX:0,offsetY:0,widthScale:1,heightScale:1,zoom:1,panX:0,panY:0});noBackground(chosen)};
  ...
  const resetImageStage=()=>{if(!imageEditor)return;const target=layers.find(l=>l.id===imageEditor.layerId),first=imageEditor.history[0];setImageEditor({...imageEditor,source:first?.source||target?.src||imageEditor.source,strokes:[],crop:{left:0,top:0,right:0,bottom:0},history:[],offsetX:first?.offsetX||0,offsetY:first?.offsetY||0,widthScale:first?.widthScale||1,heightScale:first?.heightScale||1})};
  ...
  const openSeparateLayers=async(source:string,layerId:string,base?:{x:number;y:number;w:number;h:number})=>{setWorking(true);try{const result=await findOpaqueIslands(source);if(result.parts.length<2){setNotice("No separate islands were found in this design");return}setSplitPreview({layerId,preview:result.preview,parts:result.parts,base})}finally{setWorking(false)}};
  const separateCurrentImage=async()=>{if(!imageEditor)return;const target=layers.find(l=>l.id===imageEditor.layerId);if(!target)return;const pending=imageEditor.strokes.length||Object.values(imageEditor.crop).some(Boolean),rendered=pending?await renderImageStage(imageEditor):null,l=rendered?.l||0,t=rendered?.t||0,w=rendered?.w||1,h=rendered?.h||1,base={x:target.x+target.w*(imageEditor.offsetX+imageEditor.widthScale*l),y:target.y+target.h*(imageEditor.offsetY+imageEditor.heightScale*t),w:target.w*imageEditor.widthScale*w,h:target.h*imageEditor.heightScale*h};await openSeparateLayers(rendered?.src||imageEditor.source,imageEditor.layerId,base)};
  const confirmSeparateLayers=async()=>{if(!splitPreview)return;const target=layers.find(layer=>layer.id===splitPreview.layerId);if(!target)return;setWorking(true);try{const vector=["vector","stroke"].includes(target.kind),base=splitPreview.base||target,created:Layer[]=[];for(let index=0;index<splitPreview.parts.length;index++){const part=splitPreview.parts[index],solid=vector?await silhouette(part.src,target.color,255):part.src,src=vector?await vTracerCutout(solid,target.color):solid,safety=await analyzeCutSafety(src,base.w*part.width);created.push({...target,...safety,id:uid(),name:`${target.name}_Part_${index+1}`,src,originalSrc:src,x:base.x+base.w*part.left,y:base.y+base.h*part.top,w:base.w*part.width,h:base.h*part.height,naturalW:part.naturalW,naturalH:part.naturalH,kind:vector?"vector":target.kind,parentId:undefined,innerSrc:undefined,steps:[],activeStep:-1})}setLayers(items=>{const at=items.findIndex(layer=>layer.id===target.id),next=items.filter(layer=>layer.id!==target.id);next.splice(Math.max(0,at),0,...created);return next});setSelected(created.map(layer=>layer.id));setSplitPreview(null);setCutEditor(null);setImageEditor(null);setBgEditor(null);setNotice(`${created.length} separate layers created`)}finally{setWorking(false)}};
  ...
  return (
    ...
    >
            ...
            >
              ...
              {!layers.length && (
                <div className="empty">
                  ...
                  <button className="open-saved-empty" onClick={()=>{setProjectsOpen(true);void refreshProjects(projects.length===0)}}><FolderOpen/> Open saved projects</button>
                </div>
              )}
            </div>
          </div>
      ...
      {riskLayerId&&(()=>{const risk=layers.find(layer=>layer.id===riskLayerId);if(!risk)return null;const cutout=["vector","stroke"].includes(risk.kind);return <div className="project-transition-modal cut-safety-modal" role="dialog" aria-modal="true"><div><button className="modal-x" onClick={()=>setRiskLayerId(null)}><X/></button><AlertTriangle/><h3>Cut safety issue</h3><p><b>Too thin or broken edges.</b><br/>{risk.cutRiskReason||"This layer contains details smaller than 2 mm."}</p><footer><button onClick={()=>setRiskLayerId(null)}>Cancel</button><button onClick={()=>{setRiskLayerId(null);setSelected([risk.id]);window.setTimeout(()=>cutout?openCutoutEditor(risk):openImageEditor(risk),0)}}>{cutout?"Edit Cutout":"Edit Image"}</button><button className="confirm" onClick={()=>cutout?void quickFixCutRisk():(setRiskLayerId(null),void applyBackgroundPreset("rim",risk))}><Sparkles/> {cutout?"Quick Fix":"Quick Fix: Add Rim"}</button></footer></div></div>})()}
      ...
      {imageEditor&&(()=>{const target=layers.find(l=>l.id===imageEditor.layerId);return <div className="bg-modal image-edit-modal" role="dialog" aria-modal="true" aria-label="Image editor">
        ...
        </aside></div><footer><button className="footer-separate" onClick={()=>void separateCurrentImage()}><Layers3/> Separate as Layers</button><span className="editor-undo-hint">Ctrl+Z to undo</span><button className="image-undo" disabled={!imageEditor.history.length} onClick={undoImageStage}><Undo2/> Undo</button><button className="cancel" onClick={()=>{setImageEditor(null);setBgEditor(null)}}>Cancel</button><button className="secondary-create" onClick={()=>void applyImageEdit(true)}>Create Layer</button><button className="confirm" onClick={()=>void applyImageEdit(false)}>Save Edit</button></footer></>:imageTab==="preset"?<><div className="image-preset-body"><div className="image-preset-summary"><Sparkles/><b>Apply a ready-made background preset</b><small>The preset uses the crop and edits currently visible in this window.</small></div><aside className="image-preset-list">{([{id:"image",image:"/background-presets/image-default.png",title:"Image Remove Background",sub:"Default"},{id:"rim",image:"/background-presets/image-rim.png",title:"Image Remove Background",sub:"Add Rim"},{id:"text",image:"/background-presets/text-bw.png",title:"Text Remove Background",sub:"Black and White"}] as const).map(preset=><button key={preset.id} className={imagePreset===preset.id?"active":""} onClick={()=>setImagePreset(preset.id)}><img src={preset.image} alt=""/><span><b>{preset.title}</b><small>{preset.sub}</small></span><Check/></button>)}</aside></div><footer><span className="footer-spacer"/><button className="cancel" onClick={()=>setImageTab("edit")}>Back</button><button className="confirm" onClick={()=>void applyPresetInImageEditor()}><Sparkles/> Apply Preset</button></footer></>:bgEditor?<><div className="bg-editor-body embedded-background"><div ref={bgPreviewRef} className={`bg-preview ${bgEditor.alphaView?"alpha-view":""}`} onWheel={zoomBackground} onPointerDown={startBackgroundPan} onPointerMove={moveBackgroundPan} onPointerUp={endBackgroundPan} onPointerCancel={endBackgroundPan}>{bgPreview?<div className="bg-image-wrap" style={{"--fit-w":bgImageSize.w?`${bgImageSize.w}px`:"auto","--fit-h":bgImageSize.h?`${bgImageSize.h}px`:"auto",transform:`translate(${bgEditor.panX}px,${bgEditor.panY}px) scale(${bgEditor.zoom})`} as React.CSSProperties}><img src={bgPreview} draggable={false} alt="Background removal preview" className={bgEditor.pickingColor!==null?"eyedrop-active":""} onLoad={e=>setBgImageSize(fitEditorImage(e.currentTarget,bgPreviewRef.current))} onPointerDown={startBackgroundStroke} onPointerMove={moveBackgroundStroke} onPointerUp={endBackgroundStroke} onPointerCancel={endBackgroundStroke}/></div>:<div className="bg-loading">Preparing preview…</div>}</div><aside className="bg-controls embedded-bg-controls"><section><label>Connected Area with Brush</label><div className="mode-buttons"><button className={bgEditor.mode==="remove"?"active remove":""} onClick={()=>setBgEditor({...bgEditor,mode:"remove"})}>Remove</button><button className={bgEditor.mode==="keep"?"active keep":""} onClick={()=>setBgEditor({...bgEditor,mode:"keep"})}>Keep</button></div><label>Brush Size <b>{bgEditor.brush.toFixed(1)}%</b></label><input type="range" min=".1" max="60" step=".1" value={bgEditor.brush} onChange={e=>setBgEditor({...bgEditor,brush:+e.target.value})}/><label>Color Bleed <b>{bgEditor.sensitivity}</b></label><input type="range" min="0" max="100" value={bgEditor.sensitivity} onChange={e=>setBgEditor({...bgEditor,sensitivity:+e.target.value})}/><label>Connected Distance <b>{bgEditor.connectedReach>=51?"Full":`${(bgEditor.connectedReach/10).toFixed(1)} cm`}</b></label><input type="range" min="0" max="51" value={bgEditor.connectedReach} onChange={e=>setBgEditor({...bgEditor,connectedReach:+e.target.value})}/></section><section><label>All the Color with Eyedrop</label><div className="erase-color-row"><input type="color" value={bgEditor.eraseColors[0]?.color||"#ffffff"} onChange={e=>setBgEditor({...bgEditor,eraseColors:[{color:e.target.value,sensitivity:bgEditor.eraseColors[0]?.sensitivity||30}]})}/><button onClick={()=>setBgEditor({...bgEditor,pickingColor:0})}><Pipette/> Pick</button></div><label>Color Sensitivity <b>{bgEditor.eraseColors[0]?.sensitivity||30}</b></label><input type="range" min="0" max="100" value={bgEditor.eraseColors[0]?.sensitivity||30} onChange={e=>setBgEditor({...bgEditor,eraseColors:[{color:bgEditor.eraseColors[0]?.color||null,sensitivity:+e.target.value}]})}/></section><section><label>Edge Refinement <b>{bgEditor.edgeRefine}px</b></label><input type="range" min="-12" max="12" value={bgEditor.edgeRefine} onChange={e=>setBgEditor({...bgEditor,edgeRefine:+e.target.value})}/><label>Edge Smooth <b>{bgEditor.edgeSmooth}</b></label><input type="range" min="0" max="10" value={bgEditor.edgeSmooth} onChange={e=>setBgEditor({...bgEditor,edgeSmooth:+e.target.value})}/><label>Remove Speckles <b>{bgEditor.speckles}px</b></label><input type="range" min="0" max="1000" step="5" value={bgEditor.speckles} onChange={e=>setBgEditor({...bgEditor,speckles:+e.target.value})}/></section></aside></div><footer><button className="cancel" onClick={()=>setImageTab("edit")}>Back to Edit Image</button><button className="confirm" disabled={!bgPreview||bgRendering} onClick={()=>void commitBackground()}>Apply Background Removal</button></footer></>:null}</div></div>})()}
      {cutEditor && (
